...
echo "Adding one poison job with an unknown type and max_attempts = 1"
bun --eval '
import { db, now } from "./db";

const t = now();
db.query(`
	INSERT INTO jobs (type, payload, max_attempts, available_at, created_at, updated_at)
	VALUES ($type, $payload, 1, $availableAt, $createdAt, $updatedAt)
`).run({
	$type: "missing-handler",
	$payload: JSON.stringify({ reason: "dead-letter demo poison job" }),
	$availableAt: t,
	$createdAt: t,
	$updatedAt: t,
});
...
echo "The poison job should move to dead after one failed attempt."
